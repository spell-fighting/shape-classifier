...
The following code snippet will convert the keras web file,
which is saved using web.save('kerasmodel_weight_file'),
...
input_model_file: name of the input weight file [default: 'web.h5']
...
graph definition [default: web.ascii]
...
parser.add_argument('-input_model_file', action="store", 
                    dest='input_model_file', type=str, default='web.h5')
...
parser.add_argument('-output_graphdef_file', action="store", 
                    dest='output_graphdef_file', type=str, default='web.ascii')
...
# Load keras web and rename output
...
try:
    ...
    print('''Input file specified ({}) only holds the weights, and not the web defenition.
    Save the web using mode.save(filename.h5) which will contain the network architecture
    as well as its weights. 
    If the web is saved using web.save_weights(filename.h5), the web architecture is 
